...
### 1.	确保 MySQL 版本大于 5.5.3, 确保支持 utf8mb4 字符集(6.*的版本貌似不支持)。

### 2.	创建数据库，名字随意，服务器默认配置名为 tlog_test，字符集为 utf8mb4 ，排序规则 utf8mb4_general_ci。
  ```
...
### 4.	确保 realm 服配置的 mysql 数据库地址账户密码及连接的数据库配置正确即可。

### 5.  测试字符集可用 realm 测试函数测试， Test.test64(100)。
