...
### 1.	保证realm、world、match启动时配置唯一name，配置文件vm.args

![avatar](/res/TIM截图20200302141138.jpg)

### 2.	realm、world启动完后可以通过consul查看服务状态是否正常

![avatar](/res/TIM截图20200302141206.jpg)

### 3.	保证realm服务达到跨服条件，参考k跨服参数表配置；

### 4.	按照上面的服务启动顺序开启服务；

### 5.	若想重新匹配，可以删除match服务目录下cluster_match_dets文件或执行ModTimer.re_match函数
